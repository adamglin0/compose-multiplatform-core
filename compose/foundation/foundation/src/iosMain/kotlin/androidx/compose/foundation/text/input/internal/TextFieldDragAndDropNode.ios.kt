...
import androidx.compose.ui.draganddrop.clipMetadata
import androidx.compose.ui.draganddrop.rootViewPosition
...
internal actual fun textFieldDragAndDropNode(
    ...
    return DragAndDropTargetModifierNode(
        shouldStartDragAndDrop = { dragAndDropEvent ->
            ...
            val typeIdentifiers = dragAndDropEvent.clipMetadata.typeIdentifiers
            hintMediaTypes().any { it == MediaType.All || typeIdentifiers.hasMediaType(it) }
        },
        target =
            object : DragAndDropTarget {
                override fun onDrop(event: DragAndDropEvent): Boolean {
                    dragAndDropRequestPermission(event)
                    // The content of the dropped items can only be requested while the drop
                    // is being performed. The dropped text is loaded asynchronously and
                    // inserted at the cursor, which follows the drop position in onMoved.
                ...
                override fun onMoved(event: DragAndDropEvent) =
                    onMoved?.invoke(event.rootViewPosition) ?: Unit
